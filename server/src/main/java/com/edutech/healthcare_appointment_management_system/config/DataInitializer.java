...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // 2. Only populate demo data if the database is otherwise empty
        if (doctorRepository.count() == 0 && patientRepository.count() == 0) {
            ...
            // Create Health Tips
            healthTipRepository.save(HealthTip.builder().category("Hydration").content("Drink at least 8 glasses of water a day.").build());
            healthTipRepository.save(HealthTip.builder().category("Fitness").content("Walk at least 30 minutes daily.").build());
            healthTipRepository.save(HealthTip.builder().category("Sleep").content("Get 7-8 hours of quality sleep.").build());

            // Create Campaigns
            campaignRepository.save(Campaign.builder().title("Free Vaccination").description("Get your flu shot this weekend.").targetAudience("All").campaignDate(LocalDateTime.now().plusDays(5)).build());
            campaignRepository.save(Campaign.builder().title("Heart Health Seminar").description("Learn how to keep your heart healthy.").targetAudience("Adults").campaignDate(LocalDateTime.now().plusDays(10)).build());
